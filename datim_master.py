...
def create_master_table():
    query = """
    CREATE TABLE IF NOT EXISTS datim_master (
...
def fetch_data():
    ...
    SELECT
        ...
        SUBSTRING_INDEX(SUBSTRING_INDEX(e.name, '(', -1), ',', 1) AS numerdom,
    ...
    LEFT JOIN datim_organisationUnits o ON d.orgUnit = o.id;
    """
    connection = pool.get_connection()
    cursor = connection.cursor()
    cursor.execute(query)
...
def insert_data(chunk):
    ...
    INSERT INTO datim_master (
        datim_data_id, categoryOptionComboId, categoryOptionComboName, age_group, sex, hiv_status,
...
def main():
    create_master_table()
    print('DATIM Master table created successfully....')
